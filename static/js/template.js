...
jQuery(document).ready(function () {
	...
	if (bodyElement.hasClass('booking')) {
		...
		bookingInlineDatepicker.datepicker({
			...
			mainBookingForm.find('.duration').children('input').val((diffDays >= 1 ? diffDays  : diffDays ));
			mainBookingForm.find('.duration').find('.value').text((diffDays >= 1 ? diffDays  : diffDays ));
